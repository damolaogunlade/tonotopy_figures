function [scan] = createScan(scanOpt, opt, roiCoords)
    ...
    switch software{i} % loading bold data
        ...
        case {'.nii', '.gz', '.mgh'} % FreeSurfer
            if exist('roiCoords','var')
                tmp = createFreeSurferScan(scanOpt.boldPath{i}, scanOpt.roiPath, scanOpt.fsid, roiCoords);
            else
                tmp = createFreeSurferScan(scanOpt.boldPath{i}, scanOpt.roiPath, scanOpt.fsid);
            end
        otherwise
