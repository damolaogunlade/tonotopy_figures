function [opt] = plotParams_DO(collated, opt, subject)
...
for i = 1:length(opt) 
    ...
    for i2 = 1:length(opt(i).params)
        if ~opt(i).bestSeed % if pRF estimated parameters
            ...
            if strcmp(opt(i).params{i2}, 'mu')
                tmp(tmp > 10000) = [];
            end
            
            if strcmp(opt(i).params{i2}, 'sigma')
                %  convert from bandwidth to octaves
%                 tmp = octaves;
%                 tmp(tmp > 6.64) = []; 
%                 tmp(tmp < 0.0332) = [];
                tmp(tmp > 1500) = [];
            ...
            %threshold to 15% variance explained
            tmp = tmp(extractfield(collated.pRF,'corr') > 0.15);
            tmpTitle = sprintf('Best Seed: %s', opt(i).params{i2});
        end
        
        
        subplot(opt(i).subplot(1), opt(i).subplot(2), i2); hold on;
        histogram(tmp, opt(i).bins);
...
% cd (fullfile('/Volumes/gomez/data/tonotopy/processed/', subject, '/figures'))
saveas(gcf, strcat(collated.opt.roi,'_params_hist.png'))

%% Scatter plot of mu vs sigma
sigma = extractfield(collated.pRF, 'sigma');
mu = extractfield(collated.pRF, 'mu');


figure(); 
scatter(mu(mu<20000 & mu>20 &sigma<1000),sigma(mu<20000 & mu>20 & sigma<1000))
hold on;
title("Mu vs Bandwidth");
xlabel("Mu (Hz)");
ylabel("Sigma")

% cd (fullfile('/Volumes/gomez/data/tonotopy/processed/', subject, '/figures'))
saveas(gcf, strcat(collated.opt.roi,'_mu_vs_bandwidth'),'png')
close all
