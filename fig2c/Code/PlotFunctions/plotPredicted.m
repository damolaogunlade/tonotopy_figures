...
if ~exist('nVox', 'var') || isempty(nVox) 
    nVox = randsample(find([collated.pRF.didFit]), 3);
end
...
for plotId = 1:3 
    ...
    subplot(3,1,plotId); hold on;
    a = plot(t, zscore(actual(:,nVox(plotId))), 'Color', [0.75 0.75 0.75]);
    p = plot(t, zscore(pred(:,nVox(plotId))), 'r--');
    plot(repmat(btwScan,1,2), ylim, 'g');
    legend([a,p], ["Actual Time Course", "pRF Predicted Time Course"]);
    xlabel('Time (s)');
    ylabel('Zscored BOLD');
    title(['Voxel '  num2str(nVox(plotId))]);
end
sgtitle('3 Runs') 
% title(sprintf('Voxel %d (%d)', nVox, predicted(nScan).voxID(nVox)));
% title('S2 Example Voxel' );
